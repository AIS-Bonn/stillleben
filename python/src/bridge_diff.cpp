...
PYBIND11_MODULE(libstillleben_diff_python, m)
{
    ...
    m.def("generate_sobel_valid_mask",
        [](torch::Tensor& instance_indices, torch::Tensor depth_image) {
            ...
            auto device_type = instance_indices.type().device_type();
        ...
        R"EOS(
            generate sobel valid mask for gradient for occlusion case
            Args:
                instance_indices (tensor): HxW tensor
                depth_image (tensor): HxW tensor
            Returns:
                tensor: a binary mask for the pixels valid for sobel
        )EOS");

    // dilate object mask
    m.def("dilate_object_mask",
        [](torch::Tensor& object_mask, torch::Tensor& sobel_valid_mask, torch::Tensor& coordinates) {
            ...
            auto device_type = object_mask.type().device_type();
